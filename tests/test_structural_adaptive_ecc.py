...
from .aux_tests import check_eq_files, check_eq_dir, path_test_files, tamper_file, find_next_entry, create_dir_if_not_exist
...
def test_one_file():
    ...
    filedb = path_test_files('output', 'saecc_file.db')
    fileout = path_test_files('output', 'tuxsmall.jpg')
    fileout_rec = path_test_files('output', 'rectemp', True) # temporary folder where repaired files will be placed (we expect none so this should be temporary, empty folder)
    fileres = path_test_files('results', 'test_structural_adaptive_ecc_test_one_file.db')
...
def test_one_file_tamper():
    ...
    filein = path_test_files('input', 'tuxsmall.jpg')
    filedb = path_test_files('output', 'saecc_tamper.db')
    fileout = path_test_files('output', 'tuxsmall.jpg')
    fileout2 = path_test_files('output', 'repaired/tuxsmall.jpg')
    fileout2_dir = path_test_files('output', 'repaired')
    fileres = path_test_files('results', 'test_structural_adaptive_ecc_test_one_file_tamper.db')
...
def test_dir():
    ...
    filedb = path_test_files('output', 'saecc_dir.db')
    fileout = path_test_files('output', )
    fileout_rec = path_test_files('output', 'rectemp', True) # temporary folder where repaired files will be placed (we expect none so this should be temporary, empty folder)
    fileres = path_test_files('results', 'test_structural_adaptive_ecc_test_dir.db')
    # Generate an ecc file
    assert saecc.main('-i "%s" -d "%s" --ecc_algo=3 -g -f --silent' % (filein, filedb)) == 0
    # Check that generated ecc file is correct
    startpos1 = find_next_entry(filedb, "\xFE\xFF\xFE\xFF\xFE\xFF\xFE\xFF\xFE\xFF").next() # need to skip the comments, so we detect where the first entrymarker begins
...
def test_algo():
    ...
    filein = path_test_files('input', 'tuxsmall.jpg')
    filedb = [path_test_files('output', 'saecc_algo1.db'),
                path_test_files('output', 'saecc_algo2.db'),
                path_test_files('output', 'saecc_algo3.db'),
                ]
    fileres = path_test_files('results', 'test_structural_adaptive_ecc_test_algo.db')
    fileout_rec = path_test_files('output', 'rectemp', True)
    # For each algorithm
