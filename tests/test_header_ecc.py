...
def test_one_file():
    ...
    filedb = path_test_files('output', 'hecc_file.db')
    ...
    fileres = path_test_files('results', 'test_header_ecc_test_one_file.db')
...
def test_one_file_tamper():
    ...
    filein = path_test_files('input', 'tuxsmall.jpg')
    filedb = path_test_files('output', 'hecc_tamper.db')
    fileout = path_test_files('output', 'tuxsmall.jpg')
    fileout2 = path_test_files('output', 'repaired/tuxsmall.jpg')
    fileout2_dir = path_test_files('output', 'repaired')
    fileres = path_test_files('results', 'test_header_ecc_test_one_file_tamper.db')
...
def test_dir():
    ...
    filedb = path_test_files('output', 'hecc_dir.db')
    fileout = path_test_files('output', )
    fileout_rec = path_test_files('output', 'rectemp', True) # temporary folder where repaired files will be placed (we expect none so this should be temporary, empty folder)
    fileres = path_test_files('results', 'test_header_ecc_test_dir.db')
    # Generate an ecc file
    assert hecc.main('-i "%s" -d "%s" --ecc_algo=3 -g -f --silent' % (filein, filedb)) == 0
    # Check that generated ecc file is correct
    startpos1 = find_next_entry(filedb, "\xFE\xFF\xFE\xFF\xFE\xFF\xFE\xFF\xFE\xFF").next() # need to skip the comments, so we detect where the first entrymarker begins
...
def test_algo():
    ...
    filein = path_test_files('input', 'tuxsmall.jpg')
    filedb = [path_test_files('output', 'hecc_algo1.db'),
                path_test_files('output', 'hecc_algo2.db'),
                path_test_files('output', 'hecc_algo3.db'),
                ]
    fileres = path_test_files('results', 'test_header_ecc_test_algo.db')
    fileout_rec = path_test_files('output', 'rectemp', True) # temporary folder where repaired files will be placed (we expect none so this should be temporary, empty folder)
    # For each algorithm
