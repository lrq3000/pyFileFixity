...
- High priority: fix critical spot in file size for structural_adaptive_ecc.py. Explanation: the variable encoding rate is based on the file size. Thus if both the size of the original file change and the file size backup inside the ecc file are corrupted, then the decoding will fail. Usually this should not be a problem because corruption on storage mediums do not change the size of files but only their contents, but in some rare instances it can happen (eg, if your whole HDD file inodes index crashes, and you use a data scraper software such as Photorec or IsoBuster to recover your files). Two solutions: either add an intra-ecc for file size, or compute analytically from the ecc track: indeed, if we suppose the ECC parameters are always correct (since they're provided by the user), and since we know the size of the ecc track, we should be able to analytically deduce the size of the original file! The latter solution is a lot harder to implement, but it should be resilient to any corruption case (even more than an intra-ecc on file size)! Edge case: take care of padding null bytes for the last ecc block, this can make the calculation go off.
...
- Implement multi-files ecc, which would be a generalization of PAR2: set a new configurable parameter split_stream which would fetch the stream of characters from the specified number of files. We could for example set 3 to use 1/3 of characters from each 3 files, 10 to compose the message block from 10 different files, etc. This would allow to make an ecc file that could recreate lost files from files that are still available (this also fix a bit the issue of directory tree meta-data truncation).
...
- Extend to higher galois fields like 2^16, 2^32 or even 2^128 (allows to be more resilient against huge, adversarial bursts) using optimization tricks in: Luo, Jianqiang, et al. "Efficient software implementations of large finite fields GF (2 n) for secure storage applications." ACM Transactions on Storage (TOS) 8.1 (2012): 2.
